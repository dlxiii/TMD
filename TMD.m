...
[cfile,pname]=uigetfile({'Model*', 'All TMD Model files (Model*)'; '*.*', 'All Files (*.*)'});
if cfile==0
    eval(['cd ' curdir]);
    return
end
...
if isempty(ikm)>0
    ikm=findstr(ufile,'km');
end
if isempty(ikm)>0
    ikm=findstr(Gname,'km');
end
if isempty(ikm)==0
    km=1;
end
...
if km==1
    fprintf('The model is on uniform grid in km\n');
else
    fprintf('The model is on uniform grid in lat,lon\n');
end
if km==1 && Fxy_ll==-1
 fprintf('   If grid is uniform in km the NAME of function converting\n');
...
fprintf('\n');
fprintf('Programmed by: Lana Erofeeva: ');
fprintf('Updated by : Pierre Cazenave: ');
TMD_release;
fprintf('\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ax=p3(3);
...
if exist('poscf','var')==0
 poscf=pos;
 poscf(2)=poscf(2)+(nc-1)*(bh+dh);
 poscf(1)=poscf(1)-5e-3;
 poscf(3)=poscf(3)+0.01;
 poscf(2)=poscf(2)-5e-3;
 poscf(4)=poscf(4)+0.01;
 poscf2=pos(2)-5e-3;
 bhdh=bh+dh;
 icpl=1;
end
...
posf=pos1;
posf(2)=posf(2)+posf(4)/2-0.01;
posf(4)=posf(4)/2+0.01;
...
for ic=1:np
    ...
    pos(2)=pos(2)+(bh+dh);
    if ic==3
        pos(2)=p3(2)+np/2*bh+(np/2-1)*dh+0.02;
        pos(1)=pos(1)+bw1+0.01;
    end
end
%%%%% Input/Output files %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
posf1=posf;
posf1(2)=p3(2);
posf1(4)=posf1(4)-0.015;
uicontrol('Style','Frame','units','normalized','position',posf1,...
          'BackgroundColor',bgCol);
pos3=posf1;
pos3(4)=(pos3(4)-0.02)/4;
pos3(2)=pos3(2)+0.75*posf1(4);
pos3(1)=pos3(1)+0.005;
pos3(3)=pos3(3)-0.01;
